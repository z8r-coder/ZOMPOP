...
function PlayScene2:ctor()--这里需要完全复制我的
	self.jishuqi=1
...
--其他我的文件全部替换，还有图片
function PlayScene2:move_run_bullent(zongbushu,zidanshu,shanghai,M_short,M_Monster)--发射炮弹和走路函数，新加在jianting里
...
-- 僵尸豌豆操作
function PlayScene2:bullet_short( M_short,M_Monster,re_time,time,de_time_s,shanghai)--发射射手类，被攻击怪物类，子弹个数，子弹速度，每个子弹间隔时间的修正（默认1s）
  local i = 0
  Bullet_maix = {}
  repeat
     M_bullet = bullet.new()
...
function PlayScene2:delay_bullet_short(j,M_short,M_Monster,re_time,time,de_time_s ,shanghai)--内部调用函数函数
...
function PlayScene2:OnAction(plist_name,png_name,Str_bef_Shu)
...
--建立僵尸和射手需要用到的类
function PlayScene2:Double_monister_shorter_OnAction( plist_name,png_name,Str_bef_Shu,number)
  display.addSpriteFrames(plist_name, png_name)
...
function PlayScene2:WeiZhi_UIset(  )
...
function PlayScene2:initUI()
	-- 背景图片
	display.newSprite("PlayScene3.png")
		:pos(display.cx, display.cy)
		:addTo(self)
		...
		:onButtonClicked(function()
			...
			cc.ui.UIPushButton.new(restartImages, {scale9 = false})
				:onButtonClicked(function()
					...
					display.replaceScene(playScene2, "flipX", 0.5)
...
--特效水果消除方法
function PlayScene2:SpecialFruitBoom(fruit)
...
-- 特效
function PlayScene2:SpecialEffects(fruit)
...
-- 检查是否有动画
function PlayScene2:checkIsAction(isAnimationing)
...
-- 选中水果,其他不同水果变为透明效果
function PlayScene2:FruitGetFocuse(fruit)
	for y = 1,self.yCount do
		for x = 1,self.xCount do
...
-- 恢复水果的透明效果
function PlayScene2:FadeFocuse()
	for y = 1, self.yCount do
		for x = 1, self.xCount do
			local el_fruit = self.matrix[(y - 1) * self.xCount + x]
...
-- 清楚回退的线段
function PlayScene2:ClearLine(whiteLine)
	whiteLine:removeFromParent()
	table.remove(self.LinkLines,#self.LinkLines)
end
-- 消除完成后清除所有线段
function PlayScene2:ClearAllLine()
	-- 清除连线
...
function PlayScene2:createAndDropFruit(x, y)
...
function PlayScene2:checkNextStage()
...
--------------------------------------------------
    ...
    cc.ui.UIPushButton.new(startBtnImages, {scale9 = false})
		:onButtonClicked(function(event)
			...
			local mainScene = import("app.scenes.End"):new()
			display.replaceScene(mainScene, "flipX", 0.5)
		end)
...
function PlayScene2:dropFruits(SumFruit,fruitIndex,isSpecialDrop)
	local emptyInfo = {}
	 ji=0
...
function PlayScene2:backTo(x,y)
...
function PlayScene2:clear_flag(fruit)
	--清楚左边标记
	if (fruit.x - 1) >= 1 then
...
function PlayScene2:checkIsLast(fruit)
	--检查左边的水果
	if (fruit.x - 1) >= 1 then
		local leftNeighbor = self.matrix[(fruit.y - 1) * self.xCount + fruit.x - 1]
		if (leftNeighbor.fruitIndex == fruit.fruitIndex) then
...
--判断游戏是否还能继续
function PlayScene2:checkIsContinue()
	for y=1,self.yCount do
		for x=1,self.xCount do
			fruit = self.matrix[(y - 1)*self.xCount + x]
...
-- 3个以上能玩儿，返回false
-- 3个以下不能玩儿，返回true
function PlayScene2:checkFruitSum(fruit)
	-- body
...
-- 3个以上能玩儿，返回false
-- 3个以下不能玩儿，返回true
function PlayScene2:checkThree()
...
function PlayScene2:checkActive(fruit)
	-- 检查fruit左边的水果是否有高亮
	if (fruit.x - 1) >= 1 then
		local leftNeighbor = self.matrix[(fruit.y - 1)*self.xCount + fruit.x - 1]
...
function PlayScene2:inSpecialFruiteActive()
	local flag = 0
	if self.specialFruit ~= nil then
		for _, fruit in pairs(self.specialFruit) do
			for _, pfruit in pairs(self.actives) do
...
function PlayScene2:showActivesScore(removedFruits)
	-- 只有一个高亮，取消高亮并返回
