...
function PlayScene:ctor()--这里需要完全复制我的
	self.jishuqi=1
...
--其他我的文件全部替换，还有图片
function PlayScene:move_run_bullent(zongbushu,zidanshu,shanghai,M_short,M_Monster)--发射炮弹和走路函数，新加在jianting里
...
-- 僵尸豌豆操作
function PlayScene:bullet_short( M_short,M_Monster,re_time,time,de_time_s,shanghai)--发射射手类，被攻击怪物类，子弹个数，子弹速度，每个子弹间隔时间的修正（默认1s）
  local i = 0
  Bullet_maix = {}
  repeat
     M_bullet = bullet.new()
...
function PlayScene:delay_bullet_short(j,M_short,M_Monster,re_time,time,de_time_s ,shanghai)--内部调用函数函数
...
function PlayScene:OnAction(plist_name,png_name,Str_bef_Shu)
...
--建立僵尸和射手需要用到的类
function PlayScene:Double_monister_shorter_OnAction( plist_name,png_name,Str_bef_Shu,number)
  display.addSpriteFrames(plist_name, png_name)
...
function PlayScene:WeiZhi_UIset(  )
  ...
  local M_mons = self:Double_monister_shorter_OnAction("zombie.plist","zombie.png","jiangshi_",22)
		...
		:onButtonClicked(function()
			...
			cc.ui.UIPushButton.new(restartImages, {scale9 = false})
				:onButtonClicked(function()
					...
					display.replaceScene(playScene, "flipX", 0.5)
...
--特效水果消除方法
function PlayScene:SpecialFruitBoom(fruit)
...
-- 特效
function PlayScene:SpecialEffects(fruit)
	if fruit and fruit.isSpecial == true then
		if fruit.fruitIndex == 1 then -- 紫色蘑菇闪电特效
...
-- 检查是否有动画
function PlayScene:checkIsAction(isAnimationing)
...
-- 选中水果,其他不同水果变为透明效果
function PlayScene:FruitGetFocuse(fruit)
	for y = 1,self.yCount do
		for x = 1,self.xCount do
...
-- 清楚回退的线段
function PlayScene:ClearLine(whiteLine)
	whiteLine:removeFromParent()
	table.remove(self.LinkLines,#self.LinkLines)
end
-- 消除完成后清除所有线段
function PlayScene:ClearAllLine()
	-- 清除连线
...
function PlayScene:createAndDropFruit(x, y)
...
-- 开始按钮
    ...
    cc.ui.UIPushButton.new(restartImages, {scale9 = false})
		:onButtonClicked(function(event)
			...
			local mainScene = import("app.scenes.PlayScene"):new()
			display.replaceScene(mainScene, "flipX", 0.5)
		end)
...
function PlayScene:dropFruits(SumFruit,fruitIndex,isSpecialDrop)
	local emptyInfo = {}
	 ji=0
...
function PlayScene:clear_flag(fruit)
	--清楚左边标记
	if (fruit.x - 1) >= 1 then
...
function PlayScene:checkIsLast(fruit)
	--检查左边的水果
	if (fruit.x - 1) >= 1 then
		local leftNeighbor = self.matrix[(fruit.y - 1) * self.xCount + fruit.x - 1]
		if (leftNeighbor.fruitIndex == fruit.fruitIndex) then
...
--判断游戏是否还能继续
function PlayScene:checkIsContinue()
	for y=1,self.yCount do
		for x=1,self.xCount do
			fruit = self.matrix[(y - 1)*self.xCount + x]
...
-- 3个以上能玩儿，返回false
-- 3个以下不能玩儿，返回true
function PlayScene:checkFruitSum(fruit)
	-- body
...
function PlayScene:checkActive(fruit)
	-- 检查fruit左边的水果是否有高亮
	if (fruit.x - 1) >= 1 then
		local leftNeighbor = self.matrix[(fruit.y - 1)*self.xCount + fruit.x - 1]
...
function PlayScene:inSpecialFruiteActive()
	local flag = 0
	if self.specialFruit ~= nil then
		for _, fruit in pairs(self.specialFruit) do
			for _, pfruit in pairs(self.actives) do
...
function PlayScene:showActivesScore(removedFruits)
	-- 只有一个高亮，取消高亮并返回
